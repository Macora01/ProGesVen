        ...
        with open(filepath, "r", encoding=app_config.CSV_ENCODING) as file:
            ...
            if filename == "productos.csv" and fieldnames:
                reader = csv.DictReader(
                    file, delimiter=delimiter, fieldnames=fieldnames
                )
            else:
...
def api_search_product():
    ...
    if len(code) == 5 and not code.startswith("BI"):
        ...
        if product:
            ...
            return jsonify({"success": True, "product": product, "image": image_path})

    # Validar formato del código (código original o el completo después de la transformación)
    if not (
        validate_factory_code(code, app_config) or validate_sales_code(code, app_config)
    ):
        return jsonify(
            {
                "success": False,
...
def api_save_comment_events():
    ...
    if comment:
        ...
        try:
            with open(
                filepath, "a", newline="", encoding=app_config.CSV_ENCODING
            ) as file:
                writer = csv.DictWriter(
                    file, fieldnames=fieldnames, delimiter=app_config.CSV_DELIMITER
                )
...
def get_top_products(limit=5):
        ...
        if today in filename:
            ...
            try:
                with open(filepath, "r", encoding=app_config.CSV_ENCODING) as file:
                    reader = csv.DictReader(file, delimiter=app_config.CSV_DELIMITER)
                    for row in reader:
                        product_code = row.get("cod_venta") or row.get(
                            "cod_fabrica", ""
                        )
                        if product_code:
                            if product_code in product_sales:
                                ...
                                product_info = next(
                                    (
                                        p
                                        for p in productos
                                        if p.get("cod_fabrica") == product_code
                                        or p.get("cod_venta") == product_code
                                    ),
                                    {},
                                )
    ...
    sorted_products = sorted(
        product_sales.items(), key=lambda x: x[1]["count"], reverse=True
    )
                ...
                with open(filepath, "r", encoding=app_config.CSV_ENCODING) as file:
                    ...
                    for row in rows[-5:]:
                        recent_sales.append(
                            {
                                "location": location,
                                "product": row.get("cod_venta")
                                or row.get("cod_fabrica", ""),
...
def get_sales_data_by_date_range(start_date, end_date):
        ...
        for filename in os.listdir(SALES_DIR):
            if date_str in filename:
                ...
                try:
                    with open(filepath, "r", encoding=app_config.CSV_ENCODING) as file:
                        reader = csv.DictReader(
                            file, delimiter=app_config.CSV_DELIMITER
                        )
                        for row in reader:
                            total_sales += 1
                            daily_sales += 1
    ...
    sorted_locations = dict(
        sorted(location_sales.items(), key=lambda x: x[1]["amount"], reverse=True)
    )
